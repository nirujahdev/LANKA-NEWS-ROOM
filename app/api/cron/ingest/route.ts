...
import { makeArticleHash } from '@/lib/hash';
import { extractEntities, normalizeTitle, similarityScore } from '@/lib/text';
import { summarizeEnglish, translateSummary } from '@/lib/openaiClient';
import { categorizeCluster } from '@/lib/categorize';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: err?.message || 'unknown', runId, errors }, { status: 500 });
  }
}
...
async function loadSources() {
  const { data, error } = await supabaseAdmin.from('sources').select('*').eq('enabled', true);
