...
export default function TechnologyPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setError(false);
      try {
        const data = await loadClusters(currentLanguage, null, 'technology' as CategoryType);
        if (!cancelled) {
          setIncidents(data);
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 justify-center">
          ...
          <div className="flex-1 min-w-0 max-w-3xl">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-xl font-normal text-[#202124]">Technology</h1>
            </div>
            ...
            {!loading && !error && incidents.length === 0 && (
              <div className="bg-white rounded-xl p-12 text-center text-[#5F6368] shadow-sm border border-[#E8EAED]">
                No technology news available right now.
              </div>
            )}
          </div>
